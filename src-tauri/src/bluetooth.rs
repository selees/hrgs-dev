...
use std::sync::Arc;
use tokio::time;
use uuid::Uuid; // state 모듈에서 BluetoothState 가져오기
...
pub async fn scan_bluetooth_devices(
    ...
    let adapter = adapters
        .into_iter()
        .next()
        .ok_or("No Bluetooth adapter found".to_string())?;
...
pub async fn disconnect_bluetooth(
    ...
    if peripheral.is_connected().await.map_err(|e| e.to_string())? {
        // 구독 해제
        let characteristics = peripheral.characteristics();
        if let Some(hr_characteristic) = characteristics
            .into_iter()
            .find(|c| c.uuid == HEART_RATE_MEASUREMENT_UUID)
        {
